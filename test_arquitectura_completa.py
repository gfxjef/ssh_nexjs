#!/usr/bin/env python3
"""
Test completo de la arquitectura S3 + Base de Datos
Prueba migración, modelos, procesador y endpoints
"""
...
def test_4_pdf_processor():
    ...
    try:
        ...
        # Crear PDF de prueba simple (solo para test de inicialización)
...
def test_5_integration():
    """Test 5: Integración completa (opcional - requiere servidor corriendo)"""
    logger.info("🧪 TEST 5: Integración Completa (Opcional)")
    ...
    try:
        ...
        # Test health check
        try:
            response = requests.get(f"{base_url}/pdf-manager-s3/health", timeout=5)
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    logger.info("✅ Servidor respondiendo correctamente")
                    
                    # Test listar catálogos
                    response = requests.get(f"{base_url}/pdf-manager-s3/catalogos", timeout=5)
                    if response.status_code == 200:
                        logger.info("✅ Endpoint de catálogos funcionando")
                        return True
                    else:
                        logger.warning("⚠️ Endpoint de catálogos no responde")
                else:
                    logger.warning("⚠️ Servidor responde pero con errores")
            else:
                logger.warning("⚠️ Servidor no responde correctamente")
        except requests.exceptions.RequestException:
            logger.warning("⚠️ Servidor no está corriendo - test de integración omitido")
            return True  # No es crítico para el test
            
        return False
        
    except Exception as e:
        logger.warning(f"⚠️ Test de integración omitido: {str(e)}")
        return True  # No es crítico


def test_6_cleanup():
    """Test 6: Limpieza y verificación final"""
    logger.info("🧪 TEST 6: Limpieza y Verificación Final")
...
def main():
    ...
    logger.info("🚀 INICIANDO TESTS DE ARQUITECTURA COMPLETA")
    logger.info("=" * 60)
    ...
    tests = [
        ...
        ("Integración", test_5_integration),
        ("Limpieza", test_6_cleanup)
    ]
